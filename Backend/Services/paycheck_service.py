...
# Federal tax brackets (2024, simplified for biweekly single filer)
FEDERAL_BRACKETS = [
    (503,  0.10),
    (1600, 0.12),
    (3083, 0.22),
...
STATE_TAX_RATE       = 0.05    # flat 5% state tax
...
def _calc_federal_tax(taxable: float, allowances: int) -> float:
    """Rough federal withholding using 2024 biweekly tables."""
    allowance_value = 175.0  # per-allowance amount for biweekly period (approx)
    adjusted = max(0.0, taxable - allowances * allowance_value)
    tax = 0.0
    prev = 0.0
    for bracket, rate in FEDERAL_BRACKETS:
        if adjusted <= prev:
            break
        taxable_in_bracket = min(adjusted, bracket) - prev
        tax += taxable_in_bracket * rate
        prev = bracket
    return round(tax, 2)


def calculate_paycheck(employee_id: int, pay_period_id: int, db: Session) -> dict:
    """
    Auto-calculate paycheck amounts for an employee for a given pay period.
    Returns a dict of amounts without persisting.
    """
    ...
    time_entries = (
        ...
        .filter(
            TimeEntries.employee_id == employee_id,
            TimeEntries.entry_date >= pay_period.period_start_date,
            TimeEntries.entry_date <= pay_period.period_end_date,
        )
        .all()
    )
    ...
    if emp_pos and float(emp_pos.current_hourly_rate or 0) > 0:
        ...
        # Salaried: divide annual salary by pay periods per year
        freq_map = {"weekly": 52, "bi_weekly": 26, "semi_monthly": 24, "monthly": 12}
        periods_per_year = freq_map.get(emp_pos.pay_frequency, 26)
        gross_pay = float(emp_pos.current_salary) / periods_per_year
    else:
        gross_pay = 0.0
    ...
    federal_allowances = int(tax_info.federal_allowances) if tax_info else 0
    exempt_federal = bool(tax_info.exempt_federal) if tax_info else False
    exempt_state = bool(tax_info.exempt_state) if tax_info else False

    federal_tax = 0.0 if exempt_federal else _calc_federal_tax(gross_pay, federal_allowances)
    state_tax = 0.0 if exempt_state else round(gross_pay * STATE_TAX_RATE, 2)
...
def create_paycheck(data: PaycheckCreate, db: Session):
    # Prevent duplicate paycheck for same employee + pay period
    existing = (
        db.query(Paychecks)
        .filter(
            Paychecks.employee_id == data.employee_id,
            Paychecks.pay_period_id == data.pay_period_id,
        )
        .first()
    )
    if existing:
        raise HTTPException(status_code=409, detail="Paycheck already exists for this employee and pay period")

    paycheck = Paychecks(
        employee_id=data.employee_id,
        pay_period_id=data.pay_period_id,
        check_number=data.check_number or _generate_check_number(),
...
def run_payroll(pay_period_id: int, db: Session):
    """Auto-generate paychecks for all active employees for a pay period."""
    pay_period = db.query(PayPeriods).filter(PayPeriods.pay_period_id == pay_period_id).first()
    if not pay_period:
        raise HTTPException(status_code=404, detail="Pay period not found")

    if pay_period.status not in ("open", "processing"):
        raise HTTPException(status_code=400, detail=f"Pay period is '{pay_period.status}', cannot run payroll")
    ...
    for emp in employees:
        ...
        existing = (
            db.query(Paychecks)
            .filter(Paychecks.employee_id == emp.employee_id, Paychecks.pay_period_id == pay_period_id)
            .first()
        )
        if existing:
            continue
        ...
        paycheck = Paychecks(
            ...
            **{k: v for k, v in amounts.items() if k not in ("employee_id", "pay_period_id")},
            payment_method="direct deposit",
            payment_status="processed",
            payment_date=pay_period.pay_date,
        )
        db.add(paycheck)
        created.append(paycheck)
...
def update_paycheck_status(paycheck_id: int, data: PaycheckStatusUpdate, db: Session):
    ...
    if data.payment_date:
        paycheck.payment_date = data.payment_date
